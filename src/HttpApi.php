...
/**
 * Contains all raw HTTP Client-Server API calls.
 * For room and sync handling, consider using SDNClient.
 *
 * @package SdnSdk
 */
class HttpApi {

    const API_PATH = '/_api/client/r0';
    const MEDIA_PATH = '/_api/media/r0';
     ...
     * @var int
     */
    private int $default429WaitMs;

    /**
     * @var bool
     */
    private bool $useAuthorizationHeader;
     ...
     * HttpApi constructor.
     *
     * @param string $baseUrl The server URL e.g. 'http://localhost:8008'
     ...
     *      when server returns an HTTP 429 response without a 'retry_after_ms' key.
    ...
    public function __construct(string $baseUrl, ?string $token = null, ?string $identity = null,
        ...
        if (!filter_var($baseUrl, FILTER_VALIDATE_URL)) {
            throw new SDNException("Invalid server url $baseUrl");
        }

        if (!array_get(parse_url($baseUrl), 'scheme')) {
            throw new SDNException("No scheme in server url $baseUrl");
        }
     ...
     * @param string|null $username The local part of a SDN ID.
    ...
    public function preLogin(string $walletAddress) {
        $queryDidResp = $this->send('GET', '/address/' . $walletAddress);
        $preLoginReq = array();
        if (count($queryDidResp['data']) > 0) {
            $preLoginReq["did"] = $queryDidResp['data'][0];
        } else {
            $preLoginReq["address"] = $walletAddress;
        }
        return $this->send('POST', '/did/pre_login1', $preLoginReq);
    }
    ...
    public function didLogin(string $walletAddress, string $did, string $message, string $signature, string $nonce, string $updateTime, string $appToken = "") {
        ...
        return $this->send('POST', '/did/login', $didLoginReq);
    }
     ...
     * @throws SDNException
     */
    public function createRoom(string $name, string $alias = null, bool $isPublic = false,
    ...
    public function getJoinedRooms(): array {
        return $this->send('GET', "/joined_rooms");
    }
     ...
     * @return array|string
     * @throws SDNRequestException (code=404) if the state event is not found.
     * @throws SDNException
     */
     ...
     * @param string|null $reason Optional. The reason the message was redacted.
     * @param int|null $txnId Optional. The transaction ID to use.
    ...
    /**
     * $content_type can be an image,audio or video
     * extra information should be supplied.
     *
     * @param string $roomId
     ...
     *
     * Example:
     *       $api = new HttpApi("http://example.com", $token="foobar")
    ...
    private function send(string $method, string $path, $content = null, array $queryParams = [], array $headers = [],
                          string $apiPath = self::API_PATH, bool $returnJson = true) {
        $options = [];

        $method = strtoupper($method);
        if (!in_array($method, ['GET', 'POST', 'PUT', 'DELETE'])) {
            throw new SDNException("Unsupported HTTP method: $method");
        }
        ...
        while (true) {
            try {
                ...
                throw new SDNHttpLibException($e, $method, $endpoint);
            }

            $responseBody = $response->getBody()->getContents();

            if ($response->getStatusCode() >= 500) {
                throw new SDNUnexpectedResponse($responseBody);
            }
        ...
        if ($response->getStatusCode() < 200 || $response->getStatusCode() >= 300) {
            throw new SDNRequestException($response->getStatusCode(), $responseBody);
        }
    ...
    public function mediaUpload($content, string $contentType, string $filename = null) {
        ...
        $apiPath = self::MEDIA_PATH . "/upload";
        $queryParam = [];
        if ($filename) {
    ...
    public function getDownloadUrl(string $mxcUrl): string {
        ...
        return $this->baseUrl . self::MEDIA_PATH . "/download/" . substr($mxcUrl, 6);
    }
    ...
    public function mediaDownload(string $mxcUrl, bool $allowRemote = true) {
        ...
        $apiPath = self::MEDIA_PATH . "/download/";
    ...
    public function getThumbnail(string $mxcUrl, int $width, int $height,
        ...
        $path = substr($mxcUrl, 6);
        $apiPath = self::MEDIA_PATH . "/thumbnail/";
    ...
    public function getUrlPreview(string $url, float $ts = null) {
        ...
        $apiPath = self::MEDIA_PATH . '/preview_url';
     ...
     * @throws SDNRequestException
     */
    public function getJoinedMembers(string $roomId) {
        return $this->send('GET', sprintf("/rooms/%s/joined_members", urlencode($roomId)));
    }
     ...
     *
     * @param string $roomId The room to set the rules for.
     * @param string $guestAccess Weather guests can join. One of: ["can_join", "forbidden"]
     * @return array|string
     * @throws SDNException
     */
     ...
     * @throws SDNRequestException
     */
    public function deleteDevices(array $authBody, array $devices) {
        $content = [
            'auth' => $authBody,
            'devices' => $devices
        ];
     ...
     * @param int|null $timeout Optional. The time (in milliseconds) to wait when
     *      downloading keys from remote servers.
     * @param string|null $token Optional. If the client is fetching keys as a result of
    ...
    public function whoami(): array {
        if (!$this->token) {
            throw new SDNException('Authentication required.');
        }
