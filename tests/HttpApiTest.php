...
class HttpApiTest extends BaseTestCase {
    protected string $userId = "@alice:localhost";
    protected string $roomId = '#foo:localhost';
    protected string $token = "Dp0YKRXwx0iWDhFj7lg3DVjwsWzGcUIgARljgyAip2JD8qd5dSaWcxowTKEFetPulfLijAhv8eO"
    ...
    {
        parent::setUp();
        $this->api = new HttpApi('http://example.com');
    }
    ...
    public function testGetUserTags() {
        $tagsUrl = "http://example.com/_api/client/r0/user/%40alice%3Alocalhost/rooms/%23foo%3Alocalhost/tags";
        $container = [];
    ...
    public function testAddUserTag() {
        $tagsUrl = "http://example.com/_api/client/r0/user/%40alice%3Alocalhost/rooms/%23foo%3Alocalhost/tags/foo";
        $container = [];
        $handler = $this->getMockClientHandler([new Response(200, [], '{}')], $container);
        $this->api->setClient(new Client(['handler' => $handler]));
    ...
    public function testSetAccountData() {
        $accountDataUrl = "http://example.com/_api/client/r0/user/%40alice%3Alocalhost/account_data/foo";
        $container = [];
        $handler = $this->getMockClientHandler([new Response(200, [], '{}')], $container);
        $this->api->setClient(new Client(['handler' => $handler]));
    ...
    public function testSetRoomAccountData() {
        $accountDataUrl = 'http://example.com/_api/client/r0/user/%40alice%3Alocalhost/';
        $accountDataUrl .= 'rooms/%23foo%3Alocalhost/account_data/foo';
        $container = [];
        $handler = $this->getMockClientHandler([new Response(200, [], '{}')], $container);
        $this->api->setClient(new Client(['handler' => $handler]));
    ...
    public function testUnban() {
        $unbanUrl = 'http://example.com/_api/client/r0/rooms/%23foo%3Alocalhost/unban';
        $container = [];
        $handler = $this->getMockClientHandler([new Response(200, [], '{}')], $container);
        $this->api->setClient(new Client(['handler' => $handler]));
    ...
    public function testGetDevices() {
        $getDevicesUrl = "http://example.com/_api/client/r0/devices";
        $container = [];
        $handler = $this->getMockClientHandler([new Response(200, [], '{}')], $container);
        $this->api->setClient(new Client(['handler' => $handler]));
        $this->api->getDevices();
    ...
    public function testGetDevice() {
        $getDevicesUrl = "http://example.com/_api/client/r0/devices/" . $this->deviceId;
        $container = [];
        $handler = $this->getMockClientHandler([new Response(200, [], '{}')], $container);
        $this->api->setClient(new Client(['handler' => $handler]));
    ...
    public function testDeleteDevice() {
        ...
        $this->api->setClient(new Client(['handler' => $handler]));
        $this->expectException(SDNHttpLibException::class);

        try {
            $this->api->deleteDevice($this->authBody, $this->deviceId);
        } catch (SDNHttpLibException $e) {
            /** @var Request $req */
            $req = array_get($container, '0.request');
    ...
    public function testDeleteDevices() {
        $getDevicesUrl = "http://example.com/_api/client/r0/delete_devices/";
        $container = [];
        // Test for 401 status code of User-Interactive Auth API
        $handler = $this->getMockClientHandler([new Response(401, [], '{}')], $container);
        $this->api->setClient(new Client(['handler' => $handler]));
    ...
    public function testUploadKeys(array $args) {
        $uploadKeysUrl = 'http://example.com/_api/client/r0/keys/upload';
        $container = [];
        $handler = $this->getMockClientHandler([new Response(200, [], '{}')], $container);
        $this->api->setClient(new Client(['handler' => $handler]));
    ...
    public function testQueryKeys() {
        $queryKeyUrl = 'http://example.com/_api/client/r0/keys/query';
        $container = [];
        $handler = $this->getMockClientHandler([new Response(200, [], '{}')], $container);
        $this->api->setClient(new Client(['handler' => $handler]));
    ...
    public function testClaimKeys() {
        $claimKeysUrl = 'http://example.com/_api/client/r0/keys/claim';
        $container = [];
        $handler = $this->getMockClientHandler([new Response(200, [], '{}')], $container);
        $this->api->setClient(new Client(['handler' => $handler]));
    ...
    public function testKeyChange() {
        $keyChangeUrl = 'http://example.com/_api/client/r0/keys/changes';
        $container = [];
        $handler = $this->getMockClientHandler([new Response(200, [], '{}')], $container);
        $this->api->setClient(new Client(['handler' => $handler]));
    ...
    public function testSendToDevice() {
        ...
        $sendToDeviceUrl = "http://example.com/_api/client/r0/sendToDevice/m.new_device/" . $txnId;
        $container = [];
        $handler = $this->getMockClientHandler([new Response(200, [], '{}')], $container);
        $this->api->setClient(new Client(['handler' => $handler]));
    ...
    public function testSendTokenHeader() {
        $mapi = new HttpApi("http://example.com", $this->token);
    ...
    public function testSendTokenQuery() {
        $mapi = new HttpApi("http://example.com", $this->token, null, 500, false);
    ...
    public function testSendUserId() {
        $mapi = new HttpApi("http://example.com", $this->token, $this->userId);
    ...
    public function testSendUnsupMethod() {
        $this->expectException(SDNException::class);
        $mapi = new HttpApi("http://example.com", $this->token, $this->userId);
    ...
    public function testSendRequestError() {
        $this->expectException(SDNHttpLibException::class);
        $mapi = new HttpApi("http://example.com");
        $this->invokePrivateMethod($mapi, 'send', ['GET', $this->testPath]);
    }
    ...
    public function testMediaDownload() {
        $dlUrl = "http://example.com/_api/media/r0/download/" . substr($this->mxurl, 6);
        $container = [];
    ...
    public function testGetThumbnail() {
        $dlUrl = "http://example.com/_api/media/r0/thumbnail/" . substr($this->mxurl, 6);
        $container = [];
        $response = new Response(200, [
            'Content-Type' => 'application/php'
        ], file_get_contents('./tests/TestHelper.php'));
    ...
    public function testGetUrlPreview() {
        $mediaUrl = "http://example.com/_api/media/r0/preview_url";
    ...
    public function testCreateRoomVisibility(bool $isPublic, string $visibility) {
        $createRoomUrl = "http://example.com/_api/client/r0/createRoom";
        $container = [];
    ...
    public function testWhoami() {
        $mapi = new HttpApi("http://example.com", $this->token);
        $whoamiUrl = "http://example.com/_api/client/r0/account/whoami";
